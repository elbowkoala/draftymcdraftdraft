...
m_e = .010 #9.11e-31         #Mass of electron
...
#print "Epts_e_8:",Epts_e_8
#print "Epts_p_8:",Epts_p_8
...
def evolve(t_final):
    ...
    while t < t_final:
        ...
        #del ax.collections[:]
        #ax.plot(part_pos_0_e[:,0], part_vel_0_plot_e[:,0], "k.")
        #plt.show()
        #plt.cla()
        plt.imshow(Qgrid_0, cmap="hot")
        plt.colorbar()
        plt.gca().invert_yaxis()
        #plt.pause(.0001)
        #ax.plot(part_pos_0[:,0], part_pos_0[:,1], "k.")       
        #plt.pause(.0001)
        plt.contour(X,Y,phi_0,colors='k')
        #lt.pause(.0001)
        plt.quiver(X,Y,Efx_0,Efy_0,color='y')     
        plt.xlabel('X-position',fontsize=15)
        plt.ylabel('Y-position',fontsize=15)
        plt.title('Charge density, Potential, E-field on Grid',fontsize=30)
        plt.show()
        plt.pause(.0001)
